export const checkUrl = (
  ...
  if (withPath && withProtocol) {
    return isValid(
      url.match(
        /^((https):\/\/)([\p{L}\p{N}\p{S}\-_.])+(\.?([\p{L}\p{N}]|xn--[\p{L}\p{N}-]+)+\.?)(:[0-9]+)?((\/[\p{L}\p{N}\-_.]+)+)?$/gu,
      ),
    );
  } else if (withProtocol) {
    return isValid(
      url.match(
        /^((https):\/\/)([\p{L}\p{N}\p{S}\-_.])+(\.?([\p{L}\p{N}]|xn--[\p{L}\p{N}-]+)+\.?)(:[0-9]+)?$/gu,
      ),
    );
  } else if (withPath) {
    return isValid(
      url.match(
        /^([\p{L}\p{N}\p{S}\-_.])+(\.?([\p{L}\p{N}]|xn--[\p{L}\p{N}-]+)+\.?)(:[0-9]+)?(\/[\p{L}\p{N}\-_.]+)+?$/gu,
      ),
    );
  } else {
    return isValid(
      url.match(
        /^([\p{L}\p{N}\p{S}\-_.])+(\.?([\p{L}\p{N}]|xn--[\p{L}\p{N}-]+)+\.?)(:[0-9]+)?$/gu,
      ),
    );
  }
};
...
export const checkDomain = (url: string) => {
  return isValid(
    url.match(
      /^([\p{L}\p{N}\p{S}\-_.])+(\.?([\p{L}\p{N}]|xn--[\p{L}\p{N}-]+)+\.?)?$/gu,
    ),
  );
};
